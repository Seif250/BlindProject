        ...
        <Box sx={{ minHeight: '100vh', background: 'linear-gradient(180deg, #050714 0%, #0a0f1e 100%)', pt: 12, pb: 8 }}>
            <Container maxWidth="md">
                ...
                {requests.length === 0 ? (
                    ...
                    <List>
                        {requests.map((req) => {
                            const initials = req.user?.name ? req.user.name.charAt(0).toUpperCase() : 'U';
                            return (
                                <ListItem key={req._id} sx={{ background: 'rgba(12, 17, 31, 0.82)', border: '1px solid rgba(127, 90, 240, 0.3)', borderRadius: 4, mb: 2.5, p: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
                                    ...
                                    <Box sx={{ flex: 1 }}>
                                        <Typography variant="h6" sx={{ fontWeight: 700, color: '#e2e8f0', mb: 0.5 }}>{req.user?.name || 'Unknown applicant'}</Typography>
                                        <Typography variant="body2" sx={{ color: 'rgba(226, 232, 240, 0.7)', mb: 1 }}>{req.user?.email || 'No email provided'}</Typography>
                                        ...
                                        {req.user?._id && (
                                            <Link href={`/profile/${req.user._id}`} underline="hover" sx={{ color: '#94a3b8', fontSize: '0.85rem' }}>View profile</Link>
                                        )}
                                    </Box>
